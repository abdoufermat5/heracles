...
```bash
# OBLIGATOIRE avant chaque commit (exécuter dans le conteneur)
...
from heracles_api.models.schemas.user import UserRead
...
```python
# heracles_api/core/exceptions.py
...
from heracles_api.core.exceptions import UserNotFoundError, PermissionDeniedError
from heracles_api.models.schemas.user import UserCreate, UserRead, UserUpdate
from heracles_api.models.schemas.common import PaginatedResponse
...
```python
# heracles_api/services/user_service.py
...
**IMPORTANT: Exécuter les tests dans le conteneur:**
```bash
docker compose exec api sh -c "PYTHONPATH=/app pytest -v"                     # Tous les tests
docker compose exec api sh -c "PYTHONPATH=/app pytest tests/test_auth.py"     # Fichier spécifique
docker compose exec api sh -c "PYTHONPATH=/app pytest -k 'test_login'"        # Test par nom
```
...
docker compose --profile full exec ui npm run format
```
