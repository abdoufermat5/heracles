...
| Tests compatibilité | P0 | 3j | ✅ Terminé (coexistence vérifiée) |
...
### 4.3 Sprint 17-18: DNS & DHCP (4 semaines) ✅ TERMINÉ

| Tâche | Priorité | Estimation | Statut |
|-------|----------|------------|--------|
| Plugin `dns` backend | P0 | 5j | ✅ Terminé (schemas, service, router, 8 types records) |
| LDAP schema dnszone | P0 | 1j | ✅ Terminé (dnszone.ldif) |
| UI: Gestion DNS | P0 | 4j | ✅ Terminé (zones list, zone detail, records table) |
| Plugin `dhcp` backend | P0 | 5j | ✅ Terminé (schemas, service, routes, 80 tests) |
| LDAP schema dhcp | P0 | 1j | ✅ Terminé (dhcp-heracles.schema + ldif) |
| UI: Gestion DHCP | P0 | 4j | ✅ Terminé (services, subnets, hosts, pools, tree view) |
| 11 types objets DHCP | P0 | 3j | ✅ Terminé (service, shared_network, subnet, pool, host, group, class, subclass, tsig_key, dns_zone, failover_peer) |
| Tests | P0 | 2j | ✅ Terminé (80 unit tests DHCP) |

**Livrable**: Plugin dns ✅ | Plugin dhcp ✅ | Tests complets ✅
...
✅ Plugin dns (zones, 8 types records: A, AAAA, MX, NS, CNAME, PTR, TXT, SRV) - TERMINÉ
✅ Plugin dhcp (11 types: service, shared_network, subnet, pool, host, group, class, subclass, tsig_key, dns_zone, failover_peer) - TERMINÉ
✅ Plugin mail (attributs mail sur utilisateurs et groupes) - TERMINÉ
...
## 4.5 Phase 3.5: Department Management (Q1 2026)

**Objectif**: Gestion hiérarchique des départements avec filtrage contextuel des données.

**Durée**: 1 semaine

### 4.5.1 Department Management ✅ TERMINÉ

| Tâche | Priorité | Estimation | Statut |
|-------|----------|------------|--------|
| LDAP schema hrcDepartment | P0 | 1j | ✅ Terminé (heracles-department.schema) |
| API endpoint `/departments` | P0 | 2j | ✅ Terminé (CRUD, tree API) |
| Department repository | P0 | 1j | ✅ Terminé (hierarchical queries) |
| UI: Department selector | P0 | 1j | ✅ Terminé (sidebar selector, tree popover) |
| UI: Department breadcrumbs | P0 | 1j | ✅ Terminé (clickable navigation) |
| UI: Department pages | P1 | 1j | ✅ Terminé (list, detail, create) |
| Context-aware data filtering | P0 | 1j | ✅ Terminé (users, groups, all plugins) |
| Department store (Zustand) | P0 | 1j | ✅ Terminé (persisted state) |
| Create entities in context | P0 | 1j | ✅ Terminé (users créés dans dept sélectionné) |
| Dashboard context-aware | P1 | 1j | ✅ Terminé (stats filtrées par dept) |

**Livrable**: Department Management complet ✅

### 4.5.2 Milestone Phase 3.5

```
✅ LDAP schema hrcDepartment (ou, description, parentDn)
✅ Department CRUD API (/departments, /departments/{dn}, /departments/tree)
✅ Hierarchical department tree structure
✅ Department selector in sidebar (persisted selection)
✅ Breadcrumb navigation for department hierarchy
✅ Context-aware filtering pour:
   - Users (ou=people,{dept})
   - Groups (ou=groups,{dept})
   - DNS zones (ou=dns,{dept})
   - DHCP services (ou=dhcp,{dept})
   - Systems (ou=systems,{dept})
   - Sudo roles (ou=sudoers,{dept})
   - POSIX groups (ou=groups,{dept})
✅ Entity creation respects department context
✅ Root-level isolation (root shows only root data)
✅ Dashboard shows contextual statistics
...
|--------|--------|-------------|------------|
| Incompatibilité LDAP | Élevé | Moyenne | Tests de coexistence précoces |
...
- ❌ Multi-tenancy
- ❌ Intégrations mail avancées (postfix, dovecot, etc.) - le plugin mail basique est implémenté
- ❌ Plugin Samba
