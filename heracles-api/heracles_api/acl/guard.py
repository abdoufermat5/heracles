...
class AclGuard:
    ...
    def can_attribute(
        ...
        try:
            low, high = self._reg.bitmap(permission)
            return self._acl.check_attribute(target_dn, low, high, object_type, action, attribute)
        except KeyError:
            return False
    ...
    def filter_read(
        ...
        try:
            low, high = self._reg.bitmap(permission)
            return self._acl.filter_attributes(target_dn, low, high, object_type, "read", attributes)
        except KeyError:
            return []
    ...
    def filter_write(
        ...
        try:
            low, high = self._reg.bitmap(permission)
            return self._acl.filter_attributes(target_dn, low, high, object_type, "write", attributes)
        except KeyError:
            return []
    ...
    def filter_data_read(
        ...
        """
        readable = set(self.filter_read(target_dn, object_type, list(data.keys()), permission))
    ...
    def filter_data_write(
        ...
        """
        writable = set(self.filter_write(target_dn, object_type, list(data.keys()), permission))
