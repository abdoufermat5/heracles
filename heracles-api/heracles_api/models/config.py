...
class ConfigCategory(Base):
    ...
    settings: Mapped[list["ConfigSetting"]] = relationship(
...
class ConfigSetting(Base):
    ...
    validation_rules: Mapped[Optional[Any]] = mapped_column(JSONB, nullable=True)
    options: Mapped[Optional[Any]] = mapped_column(JSONB, nullable=True)
    requires_restart: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default="false"
    )
    sensitive: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default="false"
    )
    read_only: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default="false"
    )
    ...
    depends_on: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    depends_on_value: Mapped[Optional[Any]] = mapped_column(JSONB, nullable=True)
    updated_by: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        TIMESTAMP(timezone=True), nullable=False, server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
    ...
    __table_args__ = (
        UniqueConstraint(
            "category_id", "key", name="config_settings_category_id_key_key"
        ),
...
class PluginConfig(Base):
    ...
    config_schema: Mapped[Optional[Any]] = mapped_column(JSONB, nullable=True)
    ...
    __table_args__ = (
        Index("idx_plugin_configs_enabled", "enabled"),
    )
...
class ConfigHistory(Base):
    ...
    plugin_name: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    category: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    setting_key: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
