...
class RoleRepository:
    ...
    async def _build_role_dn(self, cn: str, department_dn: str | None = None) -> str:
        """
    ...
    async def update(self, cn: str, updates: RoleUpdate) -> LdapEntry | None:
        """
    ...
    async def add_member(self, cn: str, member_dn: str) -> bool:
        ...
        await self.ldap.modify(entry.dn, {"roleOccupant": ("add", [member_dn])})
    ...
    async def remove_member(self, cn: str, member_dn: str) -> bool:
        ...
        await self.ldap.modify(entry.dn, {"roleOccupant": ("delete", [member_dn])})
    ...
    async def remove_user_from_all_roles(self, user_dn: str) -> int:
        ...
        for role in roles:
            try:
                await self.ldap.modify(role.dn, {"roleOccupant": ("delete", [user_dn])})
                count += 1
