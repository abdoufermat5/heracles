...
class TemplateRepository:
    ...
    async def get_by_id(self, template_id: uuid.UUID) -> UserTemplate | None:
        """Fetch a template by its UUID."""
        result = await self._session.execute(select(UserTemplate).where(UserTemplate.id == template_id))
        return result.scalar_one_or_none()

    async def get_by_name(self, name: str) -> UserTemplate | None:
        """Fetch a template by name."""
        result = await self._session.execute(select(UserTemplate).where(UserTemplate.name == name))
        return result.scalar_one_or_none()
    ...
    async def list_all(
        ...
        *,
        ...
        query = select(UserTemplate).order_by(UserTemplate.display_order, UserTemplate.name)
        count_query = select(func.count()).select_from(UserTemplate)

        if department_dn:
            query = query.where(UserTemplate.department_dn == department_dn)
            count_query = count_query.where(UserTemplate.department_dn == department_dn)
    ...
    async def update(
        ...
        **kwargs: Any,
        ...
        await self._session.execute(update(UserTemplate).where(UserTemplate.id == template_id).values(**updates))
        await self._session.flush()
        return await self.get_by_id(template_id)

    async def delete(self, template_id: uuid.UUID) -> bool:
        """Delete a template. Returns True if deleted."""
        result = await self._session.execute(delete(UserTemplate).where(UserTemplate.id == template_id))
