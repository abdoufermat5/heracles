...
class ConfigFieldValidation(BaseModel):
    ...
    required: bool = True
    min_value: int | float | None = Field(None, alias="minValue")
    max_value: int | float | None = Field(None, alias="maxValue")
...
class ConfigFieldResponse(BaseModel):
    ...
    depends_on_value: Any | None = Field(None, alias="dependsOnValue")
...
class ConfigHistoryEntry(BaseModel):
    ...
    plugin_name: str | None = Field(None, alias="pluginName")
    setting_key: str | None = Field(None, alias="settingKey")
    old_value: Any | None = Field(None, alias="oldValue")
...
class RdnChangeCheckRequest(BaseModel):
    ...
    base_dn: str | None = Field(None, alias="baseDn", description="Base DN (optional)")
    object_class_filter: str | None = Field(
        None, alias="objectClassFilter", description="Filter for specific objectClass"
    )
...
class RdnChangeCheckResponse(BaseModel):
    ...
    entries_dns: list[str] = Field(default_factory=list, alias="entriesDns", description="Sample of affected entry DNs")
    ...
    requires_confirmation: bool = Field(
        ..., alias="requiresConfirmation", description="Whether confirmation is required"
    )
...
class RdnMigrationRequest(BaseModel):
    ...
    new_rdn: str = Field(..., alias="newRdn")
    base_dn: str | None = Field(None, alias="baseDn")
    mode: str = Field("modrdn", description="Migration mode: 'modrdn', 'copy_delete', or 'leave_orphaned'")
    object_class_filter: str | None = Field(None, alias="objectClassFilter")
    confirmed: bool = Field(False, description="User has confirmed the migration after seeing warnings")
...
class RdnMigrationResponse(BaseModel):
    ...
    failed_entries: list[dict[str, str]] = Field(
        default_factory=list, alias="failedEntries", description="List of {dn, error} for failed entries"
    )
...
class SettingUpdateWithConfirmation(BaseModel):
    ...
    value: Any
    reason: str | None = None
    confirmed: bool = Field(False, description="User has confirmed the change after seeing warnings")
    migrate_entries: bool = Field(
        True, alias="migrateEntries", description="Whether to migrate existing entries (for RDN changes)"
    )
...
class SettingUpdateResponse(BaseModel):
    ...
    migration_check: RdnChangeCheckResponse | None = Field(None, alias="migrationCheck")
    migration_result: RdnMigrationResponse | None = Field(None, alias="migrationResult")
...
class PluginConfigUpdateResponse(BaseModel):
    ...
    requires_confirmation: bool = Field(False, alias="requiresConfirmation")
    migration_check: RdnChangeCheckResponse | None = Field(None, alias="migrationCheck")
    migrations: list[PluginMigrationResult] | None = None
